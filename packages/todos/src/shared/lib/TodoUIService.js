...
export class TodoUIService {
  ...
  async list() {
    this.store.replaceAll(await this._call({ op: "list" }));
  }
  async create(title) {
    this.store.replaceAll(await this._call({ op: "create", title }));
  }
  async toggle(id) {
    this.store.replaceAll(await this._call({ op: "toggle", id }));
  }
  async remove(id) {
    this.store.replaceAll(await this._call({ op: "delete", id }));
  }
}
