...
export class PackageViewer extends LitElement {
      ...
      <div class="wrap">
        <div class="bar">
          ...
          <select
            class="sel mono"
            .value=${this._pm}
            @change=${(e) => (this._pm = e.target.value)}
          >
        ...
        ${this._error
          ? html`<div class="card hint">Error: ${this._error}</div>`
          : ""}
        ${!this._pkg
          ? html`
              <div class="card hint">
                Set <code>ws</code> and <code>path</code> (to package.json) to
                load.
              </div>
            `
          : html`
              <div class="grid">
                <div class="card">
                  <div class="row">
                    ...
                    <div class="mono">${this._pkg.name ?? "-"}</div>
                  </div>
                  <div class="row">
                    <div class="k">version</div>
                    <div class="mono">${this._pkg.version ?? "-"}</div>
                  </div>
                  ${this._pkg.description
                    ? html` <div class="row">
                        <div class="k">description</div>
                        <div class="mono muted">${this._pkg.description}</div>
                      </div>`
                    : ""}
                  ${this._pkg.private !== undefined
                    ? html` <div class="row">
                        <div class="k">private</div>
                        <div class="mono">${String(this._pkg.private)}</div>
                      </div>`
                    : ""}
                  ${this._pkg.packageManager
                    ? html` <div class="row">
                        <div class="k">packageManager</div>
                        <div class="mono">${this._pkg.packageManager}</div>
                      </div>`
                    : ""}
                  ${this._quickButtons().length
                    ? html` <div class="row" style="margin-top:8px; gap:6px;">
                        ${this._quickButtons().map(
                          (b) => html`
                            <button
                              class="btn"
                              @click=${() => this._runCmd(b.cmd, b.label)}
                            >
                              ${b.label}
                            </button>
                          `
                ...
                <div class="card">
                  ...
                  <div class="scripts">
                    ${this._pkg?.scripts
                      ? Object.entries(this._pkg.scripts).map(
                          ([name, val]) => html`
                            <div class="row">
                              <span class="pill mono">${name}</span>
                              <span class="mono muted" title=${val}
                                >${val}</span
                              >
                            </div>
                            <div class="row">
              ...
              <div class="card term mono" id="term">
                ${this._history.length === 0
                  ? html` <span class="hint">Output will appear here.</span> `
                  : this._history.map(
                      (h) => html`
                        <div>$ ${h.cmd}</div>
                        ${h.stdout ? html`<div>${h.stdout}</div>` : ""}
                        ${h.stderr
                          ? html`<div class="stderr">${h.stderr}</div>`
                          : ""}
                        <div class="hint">
                          [exit ${h.code}${h.timeout ? " (timeout)" : ""} ·
                          ${h.ms}ms]
                        </div>
                        <div>&nbsp;</div>
                      `
  ...
  _commandForScript(name, extraArgs = "") {
    ...
    switch (this._pm) {
      case "pnpm":
        return `pnpm run ${name}${args}`;
      case "yarn":
        return `yarn ${name}${args}`; // yarn omits "run"
      case "bun":
        return `bun run ${name}${args}`;
      default:
        return `npm run ${name}${args}`;
    }
  }
  _quickButtons() {
    ...
    if (s.install)
      buttons.push({
        label: "Install",
        cmd: this._commandForScript("install"),
      });
    if (s.build)
      buttons.push({ label: "Build", cmd: this._commandForScript("build") });
    if (s.test)
      buttons.push({ label: "Test", cmd: this._commandForScript("test") });
    if (s.start)
      buttons.push({ label: "Start", cmd: this._commandForScript("start") });
    return buttons;
  }
