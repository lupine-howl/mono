...
export class FileBundleViewer extends LitElement {
  ...
  _buildUrl() {
    const url = new URL("/rpc/fsSnapshot", location.origin);
    url.searchParams.set("ws", this.ws);
    url.searchParams.set("path", this.path);
    url.searchParams.set("recursive", this._recursive ? "1" : "0");
    url.searchParams.set("include_hidden", this._includeHidden ? "1" : "0");
    url.searchParams.set("include_binary", this._includeBinary ? "1" : "0");
    url.searchParams.set("max_files", String(this._maxFiles));
    url.searchParams.set("max_bytes_total", String(this._maxBytesTotal));
    url.searchParams.set("max_bytes_per_file", String(this._maxBytesPerFile));
    return url;
  }
  ...
  async _maybeLoad(force = false) {
    ...
    this._text = null;
    try {
      const r = await fetch(this._buildUrl().toString());
